...
# Phase 5: GNN Integration
try:
    from networks.topological_gnn import TopologicalHotspotGNN, predict_node_risks
    from networks.graph_neural_network import convert_to_pyg_data
    GNN_AVAILABLE = True
except ImportError:
    GNN_AVAILABLE = False
...
class PhysicalRiskOracle:
    ...
    def __init__(self):
        ...
        # GNN Hotspot Predictor
        if GNN_AVAILABLE:
            self.topological_gnn = TopologicalHotspotGNN(in_channels=7, hidden_channels=16)
    ...
    def _predict_congestion(self, silicon_graph: CanonicalSiliconGraph) -> Dict[str, float]:
        """Predict routing congestion based on GNN-enhanced graph structure"""
        if GNN_AVAILABLE:
             # Convert to PyTorch Geometric data
             pyg_data = convert_to_pyg_data(silicon_graph)
             # Get node-level risks from GNN
             node_risks = predict_node_risks(self.topological_gnn, pyg_data)
             
             # Map back to string node IDs
             result = {}
             node_ids = list(silicon_graph.graph.nodes())
             for idx, risk in node_risks.items():
                 if idx < len(node_ids):
                     result[node_ids[idx]] = risk
             return result
